...
// +----------------------------------------------------------------------+
//
// $Id: cms_forms_edit.php,v 1.5 2010/03/08 16:44:19 sebastien Exp $
	...
	case 2:
		...
		switch ($_POST["cms_action"]) {
			case "validate" :
				...
				$xhtml = $form->replaceField($_POST['formCode'], $field);
...
switch ($step) {
		...
		$content = '
			<div id="divInfo" style="DISPLAY: none">
				<form id="analyseForm" action="'.$_SERVER["SCRIPT_NAME"].'" method="post">
					...
					<input id="fieldName" name="fieldName" value="" type="hidden" />
				</form>
	...
	case 2:
		...
		$content = '
			<div id="divInfo" style="DISPLAY: none">
				<form id="modifyForm" action="'.$_SERVER["SCRIPT_NAME"].'" method="post">
					...
					<input id="fieldName" name="fieldName" value="'.$_POST['fieldName'].'" type="hidden" />
					<input name="formCode" value="'.htmlspecialchars($_POST['formCode']).'" type="hidden" />
					<input id="fieldId" name="fieldId" value="'.$fieldId.'" type="hidden" />
		...
		$content .= '
						<td align="right"><input type="checkbox" name="required_'.$field->getID().'" value="1"'.$required.' /></td>
						<td align="center" width="130"><input type="text" name="label_'.$field->getID().'" value="'.htmlspecialchars(io::decodeEntities($field->getAttribute("label"))).'" /><input type="hidden" name="name_'.$field->getID().'" value="'.$field->getAttribute("name").'" /></td>
						<td align="center" width="130">
							<select name="type_'.$field->getID().'" onchange="viewHideOptionsButton(this,\'options_'.$field->getID().'\');">';
		...
		<div id="divDefault" style="DISPLAY: none">
			...
			<table width="100%">
				<tr>
					<td nowrap width="100%" colSpan="4"><span fckLang="DlgCMSFormsDefault">Valeur par d&egrave;faut :</span>&nbsp;<input style="WIDTH: 80%" id="defaultValue" type="text"></td>
				</tr>
			</table>
			<br />
